...
VIRTUAL_TYPE_CAPABILITIES: dict[str, list[dict[str, Any]]] = {
    "light": [
        ...
        {"name": "set_brightness", "params": {"label": "亮度", "inputs": [{"name": "value", "type": "number", "required": True}]}},
        {"name": "set_color_temp", "params": {"label": "色温", "inputs": [{"name": "kelvin", "type": "number", "required": True}]}},
    ],
    "humidifier": [
        {"name": "on", "params": {"label": "开启"}},
        {"name": "off", "params": {"label": "关闭"}},
        {"name": "set_target_humidity", "params": {"label": "目标湿度", "inputs": [{"name": "value", "type": "number", "required": True}]}},
    ],
    "air_conditioner": [
        {"name": "on", "params": {"label": "开启"}},
        {"name": "off", "params": {"label": "关闭"}},
        {"name": "set_target_temperature", "params": {"label": "目标温度", "inputs": [{"name": "value", "type": "number", "required": True}]}},
        {"name": "set_mode", "params": {"label": "模式", "inputs": [{"name": "mode", "type": "string", "required": True}]}},
    ],
    "air_purifier": [
        {"name": "on", "params": {"label": "开启"}},
        {"name": "off", "params": {"label": "关闭"}},
...
VIRTUAL_TYPE_SENSORS: dict[str, list[tuple[str, str, Any]]] = {
    ...
    "humidifier": [("power", "on/off", False), ("humidity", "%", 50), ("temperature", "°C", 22.0), ("water_level", "%", 80)],
    "air_conditioner": [("power", "on/off", False), ("temperature", "°C", 26.0)],
    "air_purifier": [("power", "on/off", False), ("pm2_5", "µg/m3", 12), ("aqi", "AQI", 12), ("temperature", "°C", 22.0), ("humidity", "%", 50)],
}
...
class VirtualAdapter(BaseAdapter):
    ...
    def register_device(self, device_id: str, name: str, device_type: str) -> Device:
        caps = [
            ...
            for c in VIRTUAL_TYPE_CAPABILITIES.get(device_type, [
                {"name": "on", "params": {"label": "开启"}},
                {"name": "off", "params": {"label": "关闭"}},
            ])
        ]
