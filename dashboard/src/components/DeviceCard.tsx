...
function SensorBadge({ name, value, unit }: { name: string; value: unknown; unit: string }) {
  ...
  return (
    <div className="flex min-h-[70px] items-center gap-2 rounded-[24px] border border-slate-200 bg-slate-100/70 px-3 py-2.5 transition-colors hover:bg-slate-50">
...
function ActiveCard({ device, onDevicesChanged }: { device: Device; onDevicesChanged?: () => void }) {
  ...
  const renderCapability = (capability: DeviceCapability) => {
    ...
    if (inputs.length === 1) {
      ...
      return (
        <div key={capability.name} className="w-full rounded-[24px] border border-slate-200 bg-slate-100/60 p-3.5">
  ...
  return (
    <div className="rounded-[24px] border-2 border-slate-200 bg-white p-5">
